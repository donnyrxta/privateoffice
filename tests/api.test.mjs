...
const MIGRATIONS=['drizzle/0000_huge_blizzard.sql','drizzle/0001_durable_telemetry.sql','drizzle/0002_production_readiness.sql','drizzle/0003_agent_credentials.sql'];
...
try{
  ...
  r=await request('/api/agent',{cookie:agentCookie});eq(r.status,200,'agent session opens assigned workspace');eq(r.body.visits.some(v=>v.id===directId),true,'contracted agent sees directly assigned visit');
