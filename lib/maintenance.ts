...
export const RETENTION_POLICY={telemetryMs:RETENTION_MS,securityEventsMs:RETENTION_MS,visitEventsMs:RETENTION_MS,visitsMs:RETENTION_MS,closedSessionsMs:RETENTION_MS,enquiriesMs:90*86400000,agentWebSessionsMs:7*86400000,agentPageActivityMs:30*86400000};
...
export async function runRetention(d1:D1Database,now=Date.now()){
  ...
  const results=await d1.batch([
    ...
    d1.prepare('DELETE FROM agent_page_activity WHERE at < ?').bind(now-p.agentPageActivityMs),
  ...
  const names=['points','observation_rejections','security_events','events','enquiries','visits','agent_sessions','agent_page_activity','agent_web_sessions','rate_limits'];
