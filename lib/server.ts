...
export async function user(){const u=await getChatGPTUser();if(!u)throw new HttpError(401,'Please sign in to continue.');return u}
export async function owner(){const u=await user();const o=await db().prepare('SELECT owner_id FROM office WHERE id=1').first<{owner_id:string}>();if(!o||o.owner_id!==u.userId)throw new HttpError(403,'Only the office owner can access this.');return u}
